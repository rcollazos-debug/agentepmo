...
OPENCODE_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, ".."))       # agents/.opencode
...
def read_project_context(project_id: str = None) -> ProjectData:
    ...
    if not project_id:
        active = _read_file(os.path.join(OPENCODE_DIR, "active-project.md"))
        project_id = _extract_field(active, "project_id", "proyecto")

    project_dir = os.path.join(OPENCODE_DIR, "projects", project_id)
...
LOGO_PATH  = os.path.join(OPENCODE_DIR, "assets", "logo-vortexbird.png")
ICONS_DIR  = os.path.join(OPENCODE_DIR, "assets", "icons")
